...
wtd_LSE<-function(z)    {
    ...
    linearModel<-lm(Y~X, weight=w)
    parameters<-linearModel$coefficients
    lamb<-exp(parameters[1])
    alp<-parameters[2]
    ans<-c(alp, lamb, mu_wtdLSE)
    return (ans)
}
ptm<-proc.time()
k=100
...
for (i in 1:k)  {
    ...
    values[i,2]=ans[2]
    values[i,3]=ans[3]
    #keep(values, k, alpha, lambda, mu, lsefun, sure=TRUE)
}
for (i in i:k-1)  {
    if (values[i,2]<0.5)   {
        values[i,]=values[i+1,]
